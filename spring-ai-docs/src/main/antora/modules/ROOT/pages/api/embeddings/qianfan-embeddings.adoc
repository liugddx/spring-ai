...
At start-time use the `XinHuoEmbeddingModel` constructor to set the  default options used for all embedding requests.
At run-time you can override the default options, using a `XinHuoEmbeddingOptions` instance as part of your `EmbeddingRequest`.
...
Next, create an `XinHuoEmbeddingModel` instance and use it to compute the similarity between two input texts:
...
The `XinHuoEmbeddingOptions` provides the configuration information for the embedding requests.
